...
// Multer setup with local fallback
let upload;
if (isS3) {
  upload = multer({
    storage: multerS3({
...
// Save annotation
router.post("/annotate/:id", protect(["admin"]), upload.single("annotatedImage"), async (req, res) => {
  try {
    ...
    submission.annotationJson[section] = JSON.parse(annotationJson);

    if (req.file) {
      const imagePath = isS3 ? req.file.location : req.file.path.replace(/\\/g, "/");
